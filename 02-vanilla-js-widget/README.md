...
| `data-livekit-src` | jsDelivr `livekit-client@2.22.3` UMD | override to self-host the SDK, or to move to a newer version you have tested |
...
3. **Load `livekit-client` from the CDN** (skipped if `window.LivekitClient` already exists). The URL pins an exact version, `2.22.3`; a floating `@2` would silently change the SDK under every deployed widget.
